      ...
      <div ref={gridRef} className="grid md:grid-cols-3 gap-10 items-start">
        {TIERS.map((tier) =>
          tier.featured ? (
            <div
              key={tier.service}
              className="relative bg-white dark:bg-[#121212] p-10 rounded-[3rem] border-2 border-accent shadow-2xl md:-mt-8 md:scale-105 group z-10"
            >
              ...
              <h3 className="text-4xl font-black mb-6 mt-4">{t(tier.title)}</h3>
            ...
            <div
              key={tier.service}
              className="relative bg-white/40 dark:bg-[#2A2A2A]/40 backdrop-blur-2xl p-10 rounded-[3rem] border border-black/5 dark:border-white/10 transition-shadow duration-500 hover:shadow-2xl hover:-translate-y-2 group"
            >
              <div className="text-accent font-black text-xs uppercase tracking-[0.2em] mb-4 opacity-70">{t(tier.eyebrow)}</div>
              <h3 className="text-3xl font-black mb-6 group-hover:text-accent transition-colors">{t(tier.title)}</h3>
              <p className="text-sm opacity-60 mb-8 leading-relaxed">{t(tier.desc)}</p>
